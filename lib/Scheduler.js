...
Scheduler.prototype.triggerTask = function(taskName, callback) {
  logger.info(`Manually triggering task ${taskName}`)
  this.fetchTask(taskName, (err, task) => {
...
Scheduler.prototype.fetchJob = function(jobId, callback) {
  logger.info('Fetching job', jobId)
  this._db.fetchJob(jobId, (err, result) => {
    callback(err, result ? result[0] : null)
  })
}
